# services/api/rag/retrieval.py — Hybrid search, RRF, reranking
import logging
import re
...
# ======================= DOCUMENT FETCH (dengan cache per-call) =======================

def fetch_document_rows(supabase: Client) -> list[dict[str, Any]]:
    """
    Ambil semua baris dokumen dari Supabase.
    Hanya pilih kolom yang dibutuhkan — BUKAN select("*") untuk hemat bandwidth.
    """
...
def _table_similar_documents(
    ...
    """Cari dokumen mirip menggunakan cosine similarity langsung di memori (fallback dari RPC)."""
    try:
...
def get_similar_documents(
    ...
    if role not in {"teacher", "admin"}:
        # User biasa: gunakan table search langsung (lebih aman, tidak percaya RPC category filter)
